...
def make_action(screen, info, step, env, prev_action):
    ...
    jump_action = 2  # Assuming 2 is the action for Mario to jump
    default_action = 1  # Assuming 1 is the action for Mario to move right

    should_jump = False
    if mario_locations:
        location, dimensions, object_name = mario_locations[0]
        mario_x, mario_y = location
        #mario_height = dimensions[1]

        # Define the x and y range to check for blocks
        #x_range = range(mario_x, mario_x + 10)
        #y_range = range(mario_y + mario_height, mario_y + mario_height - 20)  
        ...
        for enemy in enemy_locations:
            ...
            if abs(mario_x - enemy_x) <= 25:
                print(f"Mario at {mario_x} is close to enemy at {enemy_x}. Jumping!")
                should_jump = True
                break
        ...
        if not should_jump:
            for block in block_locations:
                ...
                block_y = block[0][1]
                block_name = block[2]
                #if block_x in x_range and block_y in y_range:
                #   is_hole = False
                #    break
                if block_name == 'pipe' and abs(mario_x - block_x) <= 25:
                    print(f"Mario at {mario_x} is close to block at {block_x}. Jumping!")
                    should_jump = True
                    break
